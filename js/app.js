...
function updatePenaltyList() {
  ...
  const activePlayers = Object.keys(window.loss).filter(name => window.loss[name] < 3);
...
function collectPlayers() {
  ...
  window.players = window.players.filter(name => window.loss[name] < 3);
  updatePlayersPreview();
}
...
async function checkRoundEnd() {
  ...
  if (window.players.filter(p => window.loss[p] < 3).length < 4) {
    const matchEl = document.getElementById("match");
